// ============================================================================
// Hardware Source: src/actions/aggregator.actions.ts
// Route: /api/actions/aggregator
// Version: 1.0.0 — 2026-04-09
// Why: Core Server Actions for the Autonomous Content Aggregator
// Domain: Admin Operations / Intelligence
// Env / Identity: React Server Actions
// Owner: Verixa Web
// Notes: Handles data extraction, orchestration, and AI pipeline generation. Ensure ADMIN verification.
// ============================================================================
"use server";
...
export async function deleteRawArticle(id: string) {
  try {
    await verifyAdmin();
    // Do not delete from database, to prevent duplicate fetching from sources
    await prisma.rawArticle.update({ where: { id }, data: { status: "IGNORED" } });
    return { success: true };
  } catch (err: any) {
