  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-serif text-[#1A1A1A]">Welcome back, {session?.user?.name?.split(' ')[0] || 'Consultant'}.</h1>
          <p className="text-gray-500 text-sm mt-1">Here's what is happening with your practice today.</p>
        </div>
        <Link 
          href="/dashboard/performance"
           className="bg-[#1A1A1A] text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-black transition-all flex items-center justify-center gap-2"
        >
          Upgrade for 5x Visibility <ArrowUpRight className="w-4 h-4 text-[#C29967]" />
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {metrics.map((m, idx) => {
          const Icon = m.icon;
          return (
            <div key={idx} className="bg-white p-6 rounded-3xl border border-[#f5ecd8] shadow-sm relative overflow-hidden group">
              <div className="absolute -right-4 -top-4 w-16 h-16 bg-[#F6F3F0] rounded-full group-hover:bg-[#f5ecd8] transition-colors"></div>
              <div className="relative">
                 <div className="flex items-center justify-between mb-4">
                   <div className="w-10 h-10 rounded-xl bg-[#F6F3F0] text-[#C29967] flex items-center justify-center">
                     <Icon className="w-5 h-5" />
                   </div>
                   <span className="text-xs font-bold text-green-600 bg-green-50 px-2.5 py-1 rounded-md">{m.increase}</span>
                 </div>
                 <h3 className="text-3xl font-black text-[#1A1A1A] mb-1">{m.value}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl border border-red-100 shadow-sm overflow-hidden flex flex-col">
          <div className="px-6 py-5 border-b border-red-100 flex items-center justify-between bg-red-50/30">
            <div className="flex items-center gap-2">
               ...
               <h2 className="font-bold text-lg text-[#1A1A1A]">Action Inbox</h2>
            </div>
          ...
          <div className="flex-1 flex flex-col">
             {actionRequiredBookings.length === 0 ? (
               <div className="p-12 text-center text-gray-500 flex flex-col items-center justify-center flex-1">
                 ...
                 <p className="font-bold text-[#1A1A1A]">You're all caught up!</p>
                    ...
                    return (
                      <div key={booking.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50 transition-colors">
                         <div className="flex items-center gap-4">
                           ...
                           <div>
                             <h4 className="font-bold text-[#1A1A1A]">{booking.userFirstName} {booking.userLastName} <span className="text-gray-400 font-normal ml-1">· {booking.type.title}</span></h4>
                         ...
                         <Link 
                           ...
                           className="bg-[#1A1A1A] text-white text-sm font-bold px-5 py-2.5 rounded-xl hover:bg-black transition-colors self-start sm:self-center whitespace-nowrap"
                         >
                           Take Action
                         </Link>
                      </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-6">
           <h2 className="font-bold text-lg text-[#1A1A1A] mb-5">Profile Checklist</h2>
           <div className="space-y-4">
             <Link href="/dashboard/profile" className="block w-full p-4 rounded-2xl border border-gray-100 hover:border-[#C29967]/30 hover:bg-[#FDFCFB] transition-all group">
               <h4 className="font-bold text-sm text-[#1A1A1A] group-hover:text-[#C29967] transition-colors mb-1">Add Areas of Practice</h4>
               <p className="text-xs text-gray-500">List Express Entry, Study Permits, etc.</p>
             </Link>
             <Link href="/dashboard/booking" className="block w-full p-4 rounded-2xl border border-gray-100 hover:border-[#C29967]/30 hover:bg-[#FDFCFB] transition-all group">
               <h4 className="font-bold text-sm text-[#1A1A1A] group-hover:text-[#C29967] transition-colors mb-1">Enable Booking</h4>
               <p className="text-xs text-gray-500">Connect your calendar to get paid.</p>
             </Link>
             <Link href="/dashboard/profile" className="block w-full p-4 rounded-2xl border border-gray-100 hover:border-[#C29967]/30 hover:bg-[#FDFCFB] transition-all group">
               <h4 className="font-bold text-sm text-[#1A1A1A] group-hover:text-[#C29967] transition-colors mb-1">Upload Headshot</h4>
      ...
      <div className="mt-8 bg-[#1A1A1A] text-white rounded-3xl p-8 border border-black shadow-xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
         <div className="relative z-10 flex-1">
            <h2 className="text-xl font-serif font-bold text-[#C29967] mb-2">Earnings & Commission</h2>
         ...
         <div className="relative z-10 w-full md:w-auto bg-white/5 border border-white/10 rounded-2xl p-6 text-center backdrop-blur-sm">
            ...
            <button className="w-full bg-[#C29967] text-[#1A1A1A] font-bold py-3 rounded-xl hover:bg-[#b0895c] transition-colors shadow-lg">
              Upgrade to Starter
            </button>
         </div>
      </div>
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-bold text-xl font-serif text-[#1A1A1A]">Achievements & Trust Badges</h2>
          <Link href="/dashboard/performance" className="text-sm font-bold text-[#C29967] hover:underline">View All Badges</Link>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
           {/* Earned Badge 1 */}
           <div className="bg-gradient-to-br from-[#1A1A1A] to-black rounded-3xl p-6 shadow-xl shadow-black/10 border border-black relative overflow-hidden group">
             <div className="absolute -right-4 -top-4 opacity-10 group-hover:scale-110 transition-transform"><ShieldCheck className="w-24 h-24 text-[#C29967]" /></div>
             <div className="w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center mb-4 border border-white/20">
               <ShieldCheck className="w-6 h-6 text-[#C29967]" />
             </div>
           ...
           {/* Earned Badge 2 */}
           <div className="bg-white rounded-3xl p-6 shadow-sm border border-[#C29967]/30 relative overflow-hidden">
             <div className="w-12 h-12 rounded-xl bg-[#F6F3F0] flex items-center justify-center mb-4">
               <Zap className="w-6 h-6 text-[#1A1A1A] fill-black" />
             </div>
             <h3 className="text-[#1A1A1A] font-bold mb-1">Fast Responder</h3>
             <p className="text-gray-500 text-xs">Average response time under 2 hours.</p>
             <div className="mt-4 pt-4 border-t border-[#f5ecd8]">
                <span className="text-[10px] font-bold tracking-widest uppercase text-[#C29967]">Active on profile</span>
             </div>
           </div>
           ...
           <div className="bg-gray-50 rounded-3xl p-6 border border-dashed border-gray-200 relative overflow-hidden opacity-80 hover:opacity-100 transition-opacity">
             ...
             <div className="mt-4 w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
                <div className="h-full bg-[#C29967] rounded-full w-[70%]"></div>
             </div>
           </div>
