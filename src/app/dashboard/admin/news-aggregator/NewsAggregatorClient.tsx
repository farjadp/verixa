...
export default function NewsAggregatorClient({ initialSources, initialQueue }: { initialSources: any[], initialQueue: any[] }) {
  ...
  return (
    <div className="w-full flex flex-col xl:flex-row gap-6">
      ...
      <div className="xl:w-1/3 flex flex-col gap-6">
         ...
         <div className="bg-[#111111] border border-gray-800 p-6 rounded-2xl flex flex-col gap-4 shadow-xl">
            <h3 className="text-sm font-bold text-gray-300 uppercase tracking-widest border-b border-gray-800 pb-3 flex items-center gap-2">
               <Database className="w-4 h-4 text-[#C29967]" /> Source Registry
            </h3>
            
            <div className="flex flex-col gap-3">
               <input type="text" placeholder="Source Name (e.g. CIC News)" value={newSource.name} onChange={e => setNewSource({...newSource, name: e.target.value})} className="bg-[#1A1A1A] border border-gray-800 p-3 rounded-lg text-white" />
               <input type="text" placeholder="Feed URL (RSS/XML)" value={newSource.url} onChange={e => setNewSource({...newSource, url: e.target.value})} className="bg-[#1A1A1A] border border-gray-800 p-3 rounded-lg text-gray-400 font-mono text-xs" />
         ...
         <div className="bg-[#111111] border border-gray-800 p-6 rounded-2xl flex flex-col flex-1 shadow-xl min-h-[400px]">
            <h3 className="text-sm font-bold text-gray-300 uppercase tracking-widest border-b border-gray-800 pb-3 mb-4">
               Active Nodes
            </h3>
            <div className="flex flex-col gap-3 overflow-y-auto pr-2">
               {sources.map(src => (
                  <div key={src.id} className="bg-[#1A1A1A] border border-gray-800 p-4 rounded-xl flex flex-col gap-3">
                     ...
                     <button onClick={() => handleSyncSource(src.id)} disabled={loading} className="w-full bg-[#C29967]/10 hover:bg-[#C29967]/20 border border-[#C29967]/30 text-[#C29967] p-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-colors">
      ...
      <div className="xl:w-2/3 bg-[#111111] border border-gray-800 rounded-2xl flex flex-col shadow-xl overflow-hidden min-h-[700px]">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-800 text-sm">
                {queue.length === 0 ? (
                   ...
                   queue.map(q => (
                      <tr key={q.id} className="hover:bg-[#1A1A1A] transition-colors group">
                        <td className="p-4">
                           <div className="font-bold text-gray-200 line-clamp-2">{q.title}</div>
                           <a href={q.sourceUrl} target="_blank" className="text-[10px] text-gray-500 mt-1 hover:text-[#C29967] transition-colors flex items-center gap-1">
                             Verify URL <ArrowRight className="w-3 h-3" />
                           </a>
                        </td>
                        <td className="p-4">
                           <span className="text-[10px] font-bold uppercase tracking-wider text-[#C29967] bg-[#C29967]/10 border border-[#C29967]/20 px-2 py-1 rounded line-clamp-1">
                             {q.source?.name}
                           </span>
                        </td>
