// ============================================================================
// Hardware Source: src/app/dashboard/admin/news-aggregator/NewsAggregatorClient.tsx
// Route: /dashboard/admin/news-aggregator
// Version: 1.0.0 — 2026-04-09
// Why: Client side UI for Autonomous Content Aggregator
// Domain: Admin Operations
// Env / Identity: React Client Component
// Owner: Verixa Web
// Notes: Renders queue, nodes, and handles actions for aggregator.
// ============================================================================
"use client";
...
export default function NewsAggregatorClient({ initialSources, initialQueue }: { initialSources: any[], initialQueue: any[] }) {
  ...
  const handleDeleteArticle = async (articleId: string) => {
    if (!confirm("Ignore this article? It will be removed from the queue and won't be fetched again.")) return;
    setQueue(prev => prev.map(q => q.id === articleId ? { ...q, status: "IGNORED" } : q)); // optimistic
    const res = await deleteRawArticle(articleId);
    if (!res.success) {
      setError("Failed to ignore article.");
    }
  };
                     ...
                     .map(q => (
                      <tr key={q.id} className="hover:bg-[#0F2A44] transition-colors group">
                        ...
                        <td className="p-4 text-center">
                           ...
                           {q.status === "IGNORED" && <span className="text-gray-500 font-bold text-[10px] bg-gray-800 px-2 py-1 rounded">IGNORED</span>}
