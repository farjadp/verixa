// ============================================================================
// Hardware Source: src/app/dashboard/admin/social/SocialHubClient.tsx
// Route: /dashboard/admin/social
// Version: 1.0.0 — 2026-04-09
// Why: Administrative interface for AI social engine generation and omni-channel distribution.
// Domain: Admin Operations / Intelligence
// Env / Identity: React Client Component
// Owner: Verixa Web
// Notes: Renders queue, initiates AI generation, and dispatches publishes. Requires valid PlatformSettings tokens.
// ============================================================================
"use client";
  ...
  const handleSaveManualTokens = async (platform: "linkedin" | "facebook" | "twitter") => {
    ...
    try {
      if (platform === "linkedin" && (!manualState.linkedin_access_token || !manualState.linkedin_org_id)) {
        throw new Error("Both Access Token and Org ID (URN) are required.");
      }
        ...
        <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-xs font-bold ${
          ...
          {linkedinConnected === true ? (
            ...
            <div className="flex flex-col w-full gap-2">
              ...
              {showManualConnect === "linkedin" && (
                <div className="mt-2 flex flex-col gap-2.5 bg-white p-4 rounded-xl border border-orange-200 shadow-inner">
                   <p className="text-[10px] text-gray-500 font-medium mb-1">If OAuth fails due to localhost restrictions, paste developer portal tokens here:</p>
                   <input type="text" placeholder="Access Token" className="border border-gray-200 p-2 rounded-lg text-xs w-full focus:outline-none focus:border-orange-400 font-mono" 
                    value={manualState.linkedin_access_token} onChange={e => setManualState({...manualState, linkedin_access_token: e.target.value})} />
                   <input type="text" placeholder="Organization URN (e.g., urn:li:person:...)" className="border border-gray-200 p-2 rounded-lg text-xs w-full focus:outline-none focus:border-orange-400 font-mono" 
                    value={manualState.linkedin_org_id} onChange={e => setManualState({...manualState, linkedin_org_id: e.target.value})} />
                   <button onClick={() => handleSaveManualTokens("linkedin")} className="bg-orange-600 hover:bg-orange-700 text-white rounded-lg px-3 py-2 text-xs font-bold transition-colors w-max">
                     Force Save Overrides
                   </button>
                </div>
              )}
            </div>
        ...
        <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-xs font-bold ${
          ...
          {facebookConnected === true ? (
            ...
            <div className="flex flex-col w-full gap-2">
              <div className="flex items-center">
                <ShieldX className="w-4 h-4" /> 
                <span className="ml-[2px]">FB not connected —</span>
                <button onClick={handleConnectFacebook} className="underline ml-1 hover:text-blue-900 text-blue-600">Connect via OAuth</button>
                <span className="mx-2 text-gray-300">|</span>
                <button onClick={() => setShowManualConnect(showManualConnect === "facebook" ? null : "facebook")} className={`underline hover:text-orange-900 font-medium ${showManualConnect === "facebook" ? "text-orange-900" : "text-gray-500"}`}>
                  Developer Fallback
                </button>
              </div>

              {showManualConnect === "facebook" && (
                <div className="mt-2 flex flex-col gap-2.5 bg-white p-4 rounded-xl border border-orange-200 shadow-inner">
                   <p className="text-[10px] text-gray-500 font-medium mb-1">If OAuth is unavailable, use Graph API Explorer to generate keys:</p>
                   <input type="text" placeholder="Page Access Token" className="border border-gray-200 p-2 rounded-lg text-xs w-full focus:outline-none focus:border-orange-400 font-mono" 
                    value={manualState.facebook_page_token} onChange={e => setManualState({...manualState, facebook_page_token: e.target.value})} />
                   <input type="text" placeholder="Page ID" className="border border-gray-200 p-2 rounded-lg text-xs w-full focus:outline-none focus:border-orange-400 font-mono" 
                    value={manualState.facebook_page_id} onChange={e => setManualState({...manualState, facebook_page_id: e.target.value})} />
                   <input type="text" placeholder="Page Name (Optional)" className="border border-gray-200 p-2 rounded-lg text-xs w-full focus:outline-none focus:border-orange-400" 
                    value={manualState.facebook_page_name} onChange={e => setManualState({...manualState, facebook_page_name: e.target.value})} />
                   <button onClick={() => handleSaveManualTokens("facebook")} className="bg-orange-600 hover:bg-orange-700 text-white rounded-lg px-3 py-2 text-xs font-bold transition-colors w-max">
                     Force Save Overrides
                   </button>
                </div>
              )}
