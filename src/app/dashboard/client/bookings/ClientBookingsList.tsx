...
export default function ClientBookingsList({ bookings }: { bookings: PopulatedBooking[] }) {
  ...
  return (
    <div className="space-y-6">
      {/* TABS HEADER */}
      <div className="flex gap-8 border-b border-[#f5ecd8]">
        {tabs.map((tab) => (
          <button
            key={tab}
      ...
      <div className="space-y-4">
        {filteredBookings.length === 0 ? (
          <div className="bg-white border text-center border-[#f5ecd8] rounded-3xl p-12 shadow-sm">
             <CalendarDays className="w-12 h-12 text-gray-300 mx-auto mb-4" />
             <h3 className="font-bold text-lg text-[#1A1A1A] mb-1">No {activeTab.toLowerCase()} bookings</h3>
             <p className="text-gray-500 text-sm max-w-sm mx-auto">
               When you book a consultation, it will show up here.
             </p>
             {activeTab === "Upcoming" && (
                <Link href="/search" className="mt-6 inline-block bg-[#1A1A1A] text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-black shadow-lg">
                  Find a Consultant
                </Link>
             )}
          </div>
          ...
          filteredBookings.map((booking) => (
            <div key={booking.id} className="bg-white border border-[#f5ecd8] rounded-[24px] p-6 shadow-sm flex flex-col md:flex-row gap-6 items-start md:items-center hover:shadow-md transition-all group">
               {/* Avatar */}
               <div className="w-16 h-16 rounded-[16px] bg-[#F6F3F0] flex items-center justify-center shrink-0 border border-[#f5ecd8]/50">
               ...
               <div className="flex-1 space-y-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-lg font-serif text-[#1A1A1A]">{booking.profile.fullName}</h3>
               ...
               {/* Actions */}
               <div className="w-full md:w-auto flex items-center gap-3 shrink-0 border-t border-[#f5ecd8] md:border-none pt-4 md:pt-0 mt-2 md:mt-0">
                  {booking.status === "CONFIRMED" && booking.meetingLink && (
                    <a href={booking.meetingLink} target="_blank" rel="noopener noreferrer" className="bg-[#C29967] text-[#1A1A1A] font-bold px-4 py-2.5 rounded-xl text-sm hover:bg-[#b0895c] transition-colors shadow-sm">
                      Join Meeting
                    </a>
                  )}
                  ...
                  <Link href={`/dashboard/client/bookings/${booking.id}`} className="bg-white border border-[#f5ecd8] text-[#1A1A1A] font-bold px-4 py-2.5 rounded-xl text-sm hover:bg-[#FDFCFB] hover:border-[#C29967]/30 transition-all flex items-center gap-1">
