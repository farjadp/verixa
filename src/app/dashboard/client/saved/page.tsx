...
export default async function SavedProfilesPage() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      {/* Header */}
      <div>
        <h1 className="text-3xl font-serif font-bold text-[#1A1A1A]">Compare Saved Consultants</h1>
      ...
      {savedList.length === 0 ? (
        <div className="bg-white border text-center border-[#f5ecd8] rounded-3xl p-16 shadow-sm flex flex-col items-center justify-center">
           ...
           <h3 className="font-bold text-xl font-serif text-[#1A1A1A] mb-2">No profiles saved yet</h3>
           <p className="text-gray-500 text-sm max-w-sm">
             While browsing the consultant directory, click the bookmark icon to save profiles you're interested in booking.
           </p>
           <Link href="/search" className="mt-8 bg-[#1A1A1A] text-white px-8 py-3.5 rounded-xl font-bold hover:bg-black transition-all shadow-lg shadow-black/10 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 items-start">
          ...
          {savedList.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-white rounded-[24px] border border-[#f5ecd8] p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group flex flex-col relative">
                 
                 <div className="absolute top-4 right-4 bg-white shadow-sm border border-gray-100 rounded-full p-2 text-[#C29967]" title="Saved Profile">
                    <Bookmark className="w-4 h-4 fill-current" />
                 </div>

                 {/* Head */}
                 <div className="flex flex-col items-center text-center mt-2 mb-6">
                    <div className="w-20 h-20 rounded-full bg-[#F6F3F0] flex items-center justify-center mb-4 border-2 border-white shadow-sm">
                      {/* Placeholder Avatar */}
                      <span className="text-2xl font-black font-serif text-[#C29967] uppercase">{p.fullName[0]}</span>
                    </div>
                    <div className="flex items-center justify-center gap-1.5 mb-1">
                      <h2 className="font-bold font-serif text-lg text-[#1A1A1A] leading-tight">{p.fullName}</h2>
                 ...
                 <div className="bg-gray-50 rounded-2xl p-4 space-y-3 mb-6 flex-1">
                    ...
                    <div className="flex items-start gap-3">
                       ...
                       <div>
                         <p className="text-xs font-bold text-[#1A1A1A]">Licenses & Verification</p>
                    ...
                    <div className="flex items-start gap-3">
                       ...
                       <div>
                         <p className="text-xs font-bold text-[#1A1A1A]">Top Rated (4.9)</p>
                         <p className="text-xs text-gray-500 leading-snug">Based on 12 reviews</p>
                       </div>
                    </div>

                    <div className="flex items-start gap-3">
                       <Zap className="w-4 h-4 text-[#C29967] shrink-0 mt-0.5" />
                       <div>
                         <p className="text-xs font-bold text-[#1A1A1A]">Response Time</p>
                 ...
                 {/* Price & Action */}
                 <div className="mt-auto border-t border-[#f5ecd8] pt-5">
                    ...
                    <Link href={`/consultant/${p.licenseNumber}/book`} className="block w-full text-center bg-[#1A1A1A] text-white font-bold px-6 py-3.5 rounded-xl hover:bg-[#C29967] transition-colors shadow-lg shadow-black/5">
                      Book Now
                    </Link>
                    <Link href={`/consultant/${p.licenseNumber}`} className="block w-full text-center text-xs font-bold text-gray-500 uppercase tracking-widest mt-4 hover:text-[#1A1A1A]">
                      View Full Profile
                    </Link>
                 </div>
