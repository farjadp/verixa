  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex font-sans text-[#1A1F2B]">
      ...
      <aside className="w-72 bg-white border-r border-[#e5e7eb] hidden md:flex flex-col sticky top-0 h-screen">
        ...
        {/* Logo Area */}
        <div className="h-20 flex items-center px-8 border-b border-[#e5e7eb]">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link 
                key={item.href} 
                href={item.href}
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold text-gray-500 hover:text-[#1A1F2B] hover:bg-[#ffffff] transition-colors"
